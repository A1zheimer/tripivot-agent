...
### 3.2 试点结果（en-zh，252 条 held-out，贪心解码，2026-09-08 实测）

| 指标 | baseline(SFT) | OPD-R1 | GRPO-R1 |
|---|---|---|---|
| COMET(wmt22-comet-da) | 0.7921 | **0.7984** (+0.8%) | 0.7927 (+0.1%) |
| BLEU | 0.0493 | 0.0212 (−57%) | **0.0627** (+27%) |
| chrF++ | 0.2579 | 0.2581 | **0.2622** (+1.7%) |
| 长度比 | 1.320 | 0.737 | 1.223 |
| 退化率 | 3.57% | **0.00%** | 3.17% |

**结论**：
- **GRPO 综合最优**——BLEU 较基线 +27%、chrF 最高、长度健康（1.22）、退化率下降，
  指标奖励直接优化目标的效果成立；
- **OPD 单句质量最优但欠译**——COMET 最高、退化清零，但输出长度坍缩到参考的 74%
  （14B 教师简洁风格 + 反向 KL 的 mode-seeking 特性），NLL 锚（β=0.4）未能阻止；
  改进方向：长度感知的教师筛选或 β 升至 0.6；
- 两者不互相支配：**追求忠实覆盖选 GRPO，追求流畅无误选 OPD**——这正是对比实验的价值。

训练过程记录：OPD 750 步（KL 2.16→0.33，NLL 3.09→1.4，2h21m 单卡）；
GRPO 500 步（奖励 0.418，KL 锚全程 0.001 以下=策略高度稳定）。
